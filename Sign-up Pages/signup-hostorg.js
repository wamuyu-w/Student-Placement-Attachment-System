        ...
        form.addEventListener('submit', async (e) => {
            ...
            if (!data.contactPerson) {
                showError('contactPersonError', 'Contact person name is required');
                return;
            }
            if (!data.email) {
            ...
            if (!data.physicalAddress) {
                showError('physicalAddressError', 'Physical address is required');
                return;
            }
            ...
            // Submit to server
            try {
                const response = await fetch('process-signup-host-org.php', {
                    method: 'POST',
                    headers: {
                ...
                if (result.success) {
                    ...
                    setTimeout(() => {
                        window.location.href = '../Login Pages/host-organization-login.php';
                    }, 2000);
                } else {
