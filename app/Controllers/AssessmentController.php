...
class AssessmentController extends Controller {
    ...
    public function verifyCode() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Security: Verify that this lecturer is actually assigned to this attachment
            $db = (new \App\Config\Database())->connect();
            $stmt = $db->prepare("SELECT 1 FROM supervision WHERE AttachmentID = ? AND LecturerID = ? LIMIT 1");
            $stmt->bind_param("ii", $attachmentId, $_SESSION['LecturerID']);
            $stmt->execute();
            if ($stmt->get_result()->num_rows === 0) {
                $this->json(['success' => false, 'message' => 'Unauthorized: You are not assigned as a supervisor for this student.']);
                return;
            }
            ...
            if (!$actualCode) {
                ...
                // PERFORMANCE: Release session lock before redirecting to avoid TTFB latency
                session_write_close();
    ...
    public function conduct() {
        ...
        $stmt->execute();
        if ($stmt->get_result()->num_rows === 0) {
            header("Location: " . Helpers::baseUrl('/staff/assessments?error=unauthorized_assignment'));
            exit();
        }
    ...
    public function schedule() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Prevent spamming: Check if an assessment is already scheduled
            $alreadyScheduled = $assessmentModel->getScheduled($attachmentId);
            if ($alreadyScheduled) {
                $this->json(['success' => false, 'message' => 'An assessment is already scheduled for this student.']);
                return;
            }
